...
struct
  ...
  (* Sobel-Par *)
  fun sobelGradientsPar (pix: int array, h: int, w: int, numWorkers: int) : real array * real array * real array =
    let
      val n = h * w
      ...
      fun compute (y: int, x: int) =
        ...
        in (gxVal, gyVal, Math.sqrt (gxVal*gxVal + gyVal*gyVal)) end
      val _ = Parallel.parfor (0, numWorkers) (fn wk =>
        let val lo = wk * chunkSize val hi = Int.min (n, lo + chunkSize)
            fun process i = if i >= hi then ()
              else let val y = i div w val x = i mod w
  ...
  fun nonMaxSuppressionPar (gx: real array, gy: real array, mag: real array, h: int, w: int, numWorkers: int) : real array =
    let
      val n = h * w
  ...
  fun findMaxPar (arr: real array, numWorkers: int) : real =
    let val n = Array.length arr
        ...
        fun merge i m = if i >= numWorkers then m
                        else merge (i+1) (Real.max (m, Array.sub (localMax, i)))
    in merge 0 0.0 end

  fun classifyEdgesPar (magNMS: real array, h: int, w: int, numWorkers: int) : int array =
    let
      val n = h * w
      val maxVal = findMaxPar (magNMS, numWorkers)
  ...
  fun cannyPar (pix: int array, h: int, w: int, numWorkers: int) : int array =
    let
      val blurred = gaussianBlurPar (pix, h, w, numWorkers)
      val (gx, gy, mag) = sobelGradientsPar (blurred, h, w, numWorkers)
      val magNMS = nonMaxSuppressionPar (gx, gy, mag, h, w, numWorkers)
      val labels = classifyEdgesPar (magNMS, h, w, numWorkers)
      val edges = hysteresisPar (labels, h, w, numWorkers)
    in edges end
  ...
  fun main () =
    let
      ...
      val (inFile, outFile) = case args of
          [i, outF] => (i, outF)
        | [i] => (i, "canny_edges_par.txt")
        | _ => ("image_matrix.txt", "canny_edges_par.txt")
      val numWorkers = 64
      val _ = print ("Canny Edge Detection (Parallel)\n")
      ...
      val start = Time.now ()
      val edges = cannyPar (pix, h, w, numWorkers)
